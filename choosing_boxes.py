...
def get_bqm(S):
    ...
    boxes = ['box_17', 'box_21', 'box_19'] # Define variables
    # Step through the list S and assign each number to each variable
    for i in range(len(S)):
        bqm.add_variable(boxes[i], S[i])
    print(bqm)

    bqm.add_linear_equality_constraint([(boxes[i], 1) for i in range(len(S))],
