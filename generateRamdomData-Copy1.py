...
n = 6 # we generate a 10 node path
NV=2
...
#lets not define the constraint
...
for i in range(n):
    lmd.append(Placeholder("lmd_"+str(i)))
...
for j in range(1,n):
            for m in range(NV):
                ...
                #print(items[indx])
                summa.append(distance[indx%len(distance)]*items[indx])
    ...
    for j in range(n):
        if j!=i:
            for m in range(NV):
                ...
                #print(items[indx])
                summa.append(distance[indx%len(distance)]*items[indx])
    C.append(Constraint((sum(summa)-2)**2,"node_"+str(i)+"_constaint"))  
...
for i in range(len(C)):
    ConstrainMatrix += lmd[i]*C[i]


# In[139]:


model= (Hobjective+ConstrainMatrix).compile()
